...
/*   Updated: 2025/09/09 12:35:35 by ugbrouar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_texture(t_game *game, char *str)
{
	...
	else if (!ft_strcmp(map[0], "DO") && !game->texture->door)
		game->texture->door = mlx_load_png(map[1]);
...
int	get_utils(t_game *game, int fd)
{
	...
	if (!game->texture->no || !game->texture->so
		|| !game->texture->ea || !game->texture->we || !game->texture->door)
...
int	parsing(char *path, t_game *game)
{
	...
	if (fd == -1)
		return (error(game, "Unable to open the map file"));
